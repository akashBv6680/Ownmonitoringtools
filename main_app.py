...
def handle_user_input():
    ...
    if prompt := st.chat_input("Ask about your documents..."):
                ...
                try:
                    ...
                    if 'intermediate_steps' in response and len(response['intermediate_steps']) > 0:
                        ...
                        if "retrieve_documents" in tool_used_trace:
                            tool_used = "RAG"
                            retrieved_docs_count = st.session_state.get("retrieved_docs_count", 0)
                        elif "duckduckgo_search" in tool_used_trace:
                            tool_used = "DuckDuckGo"
                        elif "calculator" in tool_used_trace:
        ...
        cursor.execute(
            ...
            (datetime.now().strftime("%Y-%m-%d %H:%M:%S"), prompt, latency, tool_used, retrieved_docs_count, error_status)
        )
        conn.commit()
        conn.close()
...
# Use st.tabs to separate the chat and the dashboard
tab_chat, tab_dashboard = st.tabs(["Chat Interface", "Monitoring Dashboard"])
